...
| H23 | MACD crossover coinciding with an Elliott-oscillator bottom = test entry; support bounce or EO > 0 = main entry (two tranches, ⅓ + ⅔) | v0.8: causal MX/E1/E2 states inside open bases, CTRL random-bar control, blend + three invalidation variants, MACD-alone baseline; H23 table in base_summary.json | pending — study run requested 23 Sep | open; pre-registered in the base-pattern doc | 2026-09-23 |
...
**Open 23 Sep:** H23 (MACD + EO two-tranche entry) built as base study v0.8 with the review page and console showing the new indicators; awaiting the `study` run (~15 min) — results go into the register and the base-pattern doc.
