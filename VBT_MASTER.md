...
| H24 | Eric's eye: does blind chart reading (no name, no dates, no future) separate the bases that go on to outperform? | `label` mode: 300 decision bars (40% random / 25% DIVUP / 15% E1 / 10% COMP / 10% ANTI, listed + delisted) + 30 repeats; Buy/Pass/Unsure on a blinded page; pre-registered scorer (permutation test on BUY − PASS excess vs SPY, repeat agreement) | pending — labelling in progress | open; bar: ≥ +5 pts, p < 0.05, n_BUY ≥ 60, sign holds on the random bars, repeat agreement ≥ 75% | 2026-09-23 |
...
**Open 23 Sep (H24):** the blind-labelling test of Eric's own pattern recognition is built (`label_set.py`, `label_template.html`, `label_score.py`, Actions mode `label`, console link on the Divergence tab) and pre-registered in the base-pattern doc; waiting for the `label` run and ~1 hour of labelling.
...
**Primary (this week):** H24 — push the `label` trigger, then label all 330 blinded charts in one or two sittings (Console → Divergence → "open the labelling page"; Y/N/U, ~1 hour), download the CSV, and hand it over for scoring without opening `label_key.csv`. Then: "update both prompts" (scheduled briefings) → unpublish Pages → vbt-data private; verify the weekday LLM verdicts (Overview tab: raw_ok).
