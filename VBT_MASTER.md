...
*Source of truth for the VBT systematic trading project. Version 1.5, 24 September 2026. Every figure below is traceable to a file in vbt-private, a project doc, or a dated conversation; where a number is an estimate it is marked as such. This documents a personal, paper-first systematic process; it is not investment advice.*
...
Two GitHub repositories. **vbt-data** holds the workflows, the fetched market data and the nightly signal snapshot (docs/signals.json, docs/index.html); it was the public shell of the project until 23 September, when the GitHub Pages deployment was removed from `fetch.yml`; on 24 September the old Pages site was unpublished and the repository made private, so nothing is published any more (the two scheduled briefings fetch signals.json with the token). **vbt-private** holds every script, rulebook, ledger and study result. All dashboards are now browsed locally through the **VBT Console** (`vbt-private/console/index.html`, launched by double-clicking `VBT Console.command` in ~/Projects/VBT, which serves ~/Projects on 127.0.0.1 only): nine tabs — overview, VBT-1 tracker, VBT-2 thesis book, options, scanners (radar, winners, exhaustion), bases (live screen, gallery), divergence (the review page embedded, strict lists per scale, chart pages), symbol charts (candles, Bollinger, MACD, RSI from data/prices.csv) and the documents/study results — all read straight from the two clones, so a `git pull` is the refresh. Each Actions run clones vbt-private with a PRIVATE_REPO_TOKEN, runs the pipeline, and commits results back. One-off jobs are started by committing a trigger file whose first word is the mode (for example `base-trigger.txt` → `fund`). Secrets live in vbt-data Actions: Alpaca paper keys, the Anthropic key, EODHD_API_KEY, PRIVATE_REPO_TOKEN.
...
**Done 23 Sep (privacy + local console):** the divergence screen (`screen` trigger mode: weekly + daily on the full liquid US universe, hourly on the open-base subset via EODHD intraday) produces ticker lists, chart pages and a self-contained review page (`div_screen/div_review.html`, Y/N/unsure labels kept in the browser, CSV export) — 277 strict names on 21 Sep. Every dashboard moved into the local VBT Console; `fetch.yml` no longer builds or deploys GitHub Pages (the nightly fetch, signals, paper engine and LLM screen are unchanged); done 24 Sep: the old Pages site unpublished and vbt-data made private, both briefing prompts fetching signals.json with the token, the H24 labels and score committed; Actions minutes now count against the private-repo allowance (2,000 min/month on GitHub's free plan; the nightly job is ~5 min, a `screen` run ~35 min, a `full` study ~45 min — roughly 400–600 min/month at the current cadence).
...
**Primary (this week):** housekeeping done 24 Sep (H24 committed, both briefing prompts carry the token, Pages unpublished, vbt-data private); remaining: verify the weekday LLM verdicts (Overview tab: raw_ok), and on 16 October rotate the PAT in four places (vbt-data secret PRIVATE_REPO_TOKEN, the `.vbt_token` file, both briefing prompts). The research programme's honest state after H17–H24: nothing beats the benchmark out of the wide universe, including the operator's eye; the only live claim is selection (VBT-1/VBT-2 forward record) — leave it running and stop adding entry ideas until it has twelve months.
...
vbt-private (browsing): console/index.html (launcher `VBT Console.command` kept in ~/Projects/VBT, outside git) · div_screen/ (div_screen.csv/json/md, chart PNGs, div_series.json, div_review.html, div_review_template.html) · gallery/ · div_screen.py · intraday_fetch.py.
vbt-data (private since 24 Sep): .github/workflows (14) · data/prices.csv · docs/signals.json, docs/index.html · trigger files.
